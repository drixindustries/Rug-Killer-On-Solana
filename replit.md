...
- **Data Accuracy Improvements (Nov 2024)**: 
  ...
  - **Advanced Holder Filtering**: Multi-layered filtering system that excludes non-whale addresses from concentration calculations:
    - **LP Addresses**: DEX pairs, bonding curves, token accounts (Raydium, Orca, Meteora, Pump.fun)
  ...
  - **Pump.fun Liquidity Detection**: Special handling for Pump.fun tokens using `lpLockedPct` (100% locked in bonding curve) instead of `lpBurn` (null until Raydium graduation). Supports both `pump_fun` and `pump_fun_amm` market types from Rugcheck API. Correctly shows liquidity as SAFE with locked status when lpLockedPct >= 90%.
